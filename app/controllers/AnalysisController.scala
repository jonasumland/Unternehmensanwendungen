...
JOIN SAPHPB.MARA m
ON m.MATNR=ac.MATNR
WHERE BUDAT BETWEEN '$startDate' AND '$endDate' AND UPPER(ac.KUNNR) LIKE (UPPER('%$kundenNr%'))  AND ac.RACCT = '0041000000' AND MANDT='400'
...
""")
  ...
  def umsatzRegion(region: String, startDate: String, endDate: String): Vector[Map[String, Object]] = {
    val origset7 = sqlRunner.runSql(
      s"""SELECT TOP 10 -SUM(ac.KSL) AS UMSATZ, REGIO AS REGION, RKCUR AS WAEHRUNG  FROM SAPHPB.ACDOCA_VIEW ac
JOIN SAPHPB.KNA1 k
ON k.KUNNR = ac.KUNNR
WHERE BUDAT BETWEEN '$startDate' AND '$endDate' AND ac.RACCT = '0041000000' AND REGIO LIKE(UPPER('%$region%')) AND MANDT='400'
...
""")
  ...
  def umsatzLand(land: String, startDate: String, endDate: String): Vector[Map[String, Object]] = {
    val origset8 = sqlRunner.runSql(
      s"""SELECT TOP 10 -SUM(ac.KSL) AS UMSATZ, LAND1 as LAND, RKCUR AS WAEHRUNG  FROM SAPHPB.ACDOCA_VIEW ac
JOIN SAPHPB.KNA1 k
ON k.KUNNR = ac.KUNNR
WHERE BUDAT BETWEEN '$startDate' AND '$endDate' AND ac.RACCT = '0041000000' AND LAND1 LIKE(UPPER('%$land%')) AND MANDT='400'
